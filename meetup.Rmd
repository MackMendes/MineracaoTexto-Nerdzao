...
tw_tf_sccp <- tw_sccp[[1]] %>%
  select(created_at, text) %>%
  mutate(clube = "corinthians")
tw_tf_sccp <- tw_tf_sccp[,c(3,2,1)]


tw_tf_sccp <- tw_tf_sccp %>%
  unnest_tokens(term, text) %>%
  count(created_at, term, sort = TRUE) %>%
  ungroup()



total_tw_tf_sccp <- tw_tf_sccp %>% 
  group_by(created_at) %>% 
  summarize(total = sum(n))


tw_tf_sccp <- left_join(tw_tf_sccp, total_tw_tf_sccp)
